...
- **团队与企业** —— 用自己的域名给全员开邮箱，数据 100% 留在自己手里
...
- **三栏布局** — 文件夹、邮件列表、阅读区并排展示，列表宽度能拖到你要的
  尺寸；阅读区像 mainstream webmail 一样折叠引用内容，长帖一目了然
...
- 邮件引擎可插拔，统一走引擎无关的目录契约（`/stack/directory/*`）：
  **mailezine**（单 Go 二进制，dev/企业档默认）提供 SMTP/IMAP/POP3/
  ManageSieve，KV + blob 存储均可插拔；**postdove**（legacy MTA/IMAP stack，
  前置 nginx 网关）支撑社区版；reference server 适配器也可接入同一契约
