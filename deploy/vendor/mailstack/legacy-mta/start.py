...
def start_podop():
    system.drop_privs_to('legacy MTA')
    os.makedirs('/dev/shm/legacy MTA',mode=0o700, exist_ok=True)
    url = "http://" + os.environ["ADMIN_ADDRESS"] + ":8080/internal/legacy MTA/"
    # TODO: Remove verbosity setting from Podop?
    run_server(0, "legacy MTA", "/tmp/podop.socket", [
...
# legacy MTA requires IPv6 addresses to be wrapped in square brackets
if 'RELAYNETS' in os.environ:
    os.environ["RELAYNETS"] = re.sub(r'([0-9a-fA-F]+:[0-9a-fA-F:]+)/', '[\\1]/', os.environ["RELAYNETS"])

for postfix_file in glob.glob("/conf/*.cf"):
    conf.jinja(postfix_file, os.environ, os.path.join("/etc/legacy MTA", os.path.basename(postfix_file)))
...
for map_file in glob.glob("/overrides/*.map"):
    destination = os.path.join("/etc/legacy MTA", os.path.basename(map_file))
...
for policy in ['tls_policy', 'transport']:
    if not os.path.exists(f'/etc/legacy MTA/{policy}.map.lmdb'):
        open(f'/etc/legacy MTA/{policy}.map', 'a').close()
        os.system(f'postmap /etc/legacy MTA/{policy}.map')

if "RELAYUSER" in os.environ:
    path = "/etc/legacy MTA/sasl_passwd"
...
# Run Podop and legacy MTA
multiprocessing.Process(target=start_podop).start()
multiprocessing.Process(target=start_mta_sts_daemon).start()
os.system("/usr/libexec/legacy MTA/post-install meta_directory=/etc/legacy MTA create-missing")
# Before starting legacy MTA, we need to check permissions on /queue
# in the event that legacy MTA,postdrop id have changed
os.system("legacy MTA set-permissions")
cmd = ['legacy MTA', 'start-fg']
