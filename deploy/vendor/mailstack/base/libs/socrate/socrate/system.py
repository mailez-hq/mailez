...
def set_env(required_secrets=[], log_filters=[]):
    ...
    if _is_compatible_with_hardened_malloc():
        if not 'LD_PRELOAD' in os.environ and _is_compatible_with_hardened_malloc():
            log.warning('Your CPU has Advanced Vector Extensions available, we recommend you enable hardened-malloc earlier in the boot process by adding LD_PRELOAD=/usr/lib/libhardened_malloc.so to your mailez.env')
...
def drop_privs_to(username='mailez'):
