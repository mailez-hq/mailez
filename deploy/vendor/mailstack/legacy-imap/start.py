...
def start_podop():
    ...
    url = "http://" + os.environ["ADMIN_ADDRESS"] + ":8080/internal/legacy IMAP/§"
    run_server(0, "legacy IMAP", "/tmp/podop.socket", [
		("quota", "url", url ),
		("auth", "url", url),
		("sieve", "url", url),
    ])
...
for dovecot_file in glob.glob("/conf/*.conf"):
    conf.jinja(dovecot_file, os.environ, os.path.join("/etc/legacy IMAP", os.path.basename(dovecot_file)))
...
# Run Podop, then legacy MTA
os.system("chown mail:mail /mail")
os.system("chown -R mail:mail /var/lib/legacy IMAP /conf")

multiprocessing.Process(target=start_podop).start()
cmd = ['/usr/sbin/legacy IMAP', '-c', '/etc/legacy IMAP/dovecot.conf', '-F']
