...
- Internal API consumed by the mail stack via mainstream suite's podop bridge: legacy MTA maps,
  legacy IMAP passdb/userdb/quota/sieve, nginx auth, client autoconfig, SRS, rate limit
...
`backend/internal/internalapi` so upgrades of the mainstream suite mail images cannot break
the integration silently.

Deployment: `deploy/legacy-stack.env` + `deploy/docker-compose.yml` wire the mainstream suite mail
images (nginx/legacy MTA/IMAP stack/rspamd) to the mailess backend internal API at
`http://backend:8080` (the port the mainstream suite images hardcode). The backend listens
...
(admin, basePath `/admin`) and `127.0.0.1:8083` (webmail). The mainstream suite front
...
protocols to the backend. To run locally, copy `deploy/legacy-stack.env.example` to
`deploy/legacy-stack.env` (gitignored) and adjust `SECRET_KEY`, `DOMAIN` and
