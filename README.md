...
- **Three-pane layout** — folders, list and reading pane side by side; the
  ...
  collapses quoted replies modern-style so long threads stay scannable
...
| **community** | legacy MTA/IMAP stack | MySQL + maildir |
...
- Mail engine is pluggable behind an engine-agnostic directory contract
  ...
  KV + blob storage; **postdove** (legacy MTA/IMAP stack behind an nginx gateway)
  powers the community edition; a reference server adapter can slot in behind the
  same API
