...
这会启动 redis / gateway(nginx) / legacy IMAP / legacy MTA /
...
nginx/legacy IMAP 代理；nginx 1.26+ 的 mail auth 模块只接受 IP 字面量，所以
必须是固定容器 IP（compose 里 legacy IMAP/legacy MTA 已配静态地址，见
...
该 profile 把 legacy MTA/IMAP stack 移入 `postdove` profile（默认不带 profile 时
行为不变），启动单个 mailezine 容器（固定 IP 192.168.206.7）替代两者；
gateway 自动换用 `mailez/caddy:local` 镜像（仅 caddy，无 nginx/legacy IMAP；
...
legacy IMAP passdb、legacy MTA 查询都走它）。开发模式下 `MAILEZ_BACKEND_ADDRESS` 被
...
邮件组件（nginx / legacy IMAP / legacy MTA / rspamd / macro-scanner / unbound）的
