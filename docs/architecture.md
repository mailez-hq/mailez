...
1. **Mail stack (Docker)** — nginx gateway, legacy MTA (MTA), legacy IMAP
   (mail-keeper), Rspamd (mail-filter), oletools (macro-scanner), Unbound
...
├── stack/         internal API contract for nginx/legacy MTA/IMAP stack/rspamd
...
- The mail stack authenticates through `/internal/*`: nginx's mail proxy and
  legacy IMAP's passdb call the backend, which validates credentials, 2FA state,
