...
// TabEscape escapes bytes using legacy IMAP's tabescape (strescape.c).
...
// TabUnescape reverses legacy IMAP's tabescape.
...
// DictHandler serves the legacy IMAP dict protocol (podop replacement) over a
...
func (h *DictHandler) Serve(ctx context.Context, socketPath string) error {
	...
	// legacy IMAP connects as the mail user; the agent runs as root.
		...
		case 'L': // L<key>\t[user]
			if len(parts) >= 1 && tableURL != "" {
				...
				if val, ok := h.lookup(tableURL, string(parts[0]), lu, valueType); ok {
					...
					// legacy IMAP's dict protocol replies "O<value>\n" (no tab);
					// the vendored podop's "O\t<value>" is tolerated by some
					// legacy IMAP builds but parsed as an empty value by others.
...
// PostfixSocketmapServe serves the legacy MTA socketmap protocol (podop
...
func PostfixSocketmapServe(ctx context.Context, socketPath string, urlFunc func(table, key string) string, client *http.Client) error {
	...
	// legacy MTA connects as its own user while the agent runs as root.
	_ = os.Chmod(socketPath, 0o666)
	defer ln.Close()
...
// value legacy MTA expects, mirroring podop's str(json.loads(body)) behavior for
