...
// Contract tests for the /stack API consumed by the mail images (legacy MTA /
// legacy IMAP / nginx). The response formats below must stay byte-compatible with
...
func newContractHarness(t *testing.T) (*Handler, *fiber.App) {
	...
	h := New(db, mgr, cfg, nil, ldap.New(db, cfg.SecretKey))
	app := fiber.New()
...
func TestPostfixDomainContract(t *testing.T) {
	...
	if code, body := doGet(t, app, "/stack/legacy-mta/domain/example.com"); code != 200 || body != `"example.com"` {
		t.Fatalf("domain: got %d %q", code, body)
	}
	if code, body := doGet(t, app, "/stack/legacy-mta/domain/alt.example.com"); code != 200 || body != `"example.com"` {
		t.Fatalf("alternative: got %d %q", code, body)
	}
	if code, _ := doGet(t, app, "/stack/legacy-mta/domain/%5B1.2.3.4%5D"); code != 404 {
		t.Fatalf("bracketed domain: got %d", code)
	}
	if code, _ := doGet(t, app, "/stack/legacy-mta/domain/unknown.example.com"); code != 404 {
...
func TestPostfixMailboxContract(t *testing.T) {
	...
	if code, body := doGet(t, app, "/stack/legacy-mta/mailbox/alice@example.com"); code != 200 || body != `"alice@example.com"` {
		t.Fatalf("mailbox: got %d %q", code, body)
	}
	if code, _ := doGet(t, app, "/stack/legacy-mta/mailbox/nobody@example.com"); code != 404 {
...
func TestPostfixAliasContract(t *testing.T) {
	...
	cases := []struct {
		...
		{"/stack/legacy-mta/alias/team@example.com", `"alice@example.com,bob@example.com"`},
		{"/stack/legacy-mta/alias/team+detail@example.com", `"alice+detail@example.com,bob+detail@example.com"`},
		{"/stack/legacy-mta/alias/team%2Bdetail@example.com", `"alice+detail@example.com,bob+detail@example.com"`},
		{"/stack/legacy-mta/alias/alice@example.com", `"alice@example.com"`},
		{"/stack/legacy-mta/alias/example.com", `"example.com"`},
		{"/stack/legacy-mta/alias/wildcard@example.com", `"alice@example.com"`},
	}
	for _, c := range cases {
	...
	for _, path := range []string{
		"/stack/legacy-mta/alias/nobody@unknown.example.com",
		"/stack/legacy-mta/alias/%22quoted%22@example.com",
		"/stack/legacy-mta/alias/a%40b@example.com",
	} {
...
func TestPostfixTransportContract(t *testing.T) {
	...
	cases := []struct {
		...
		{"/stack/legacy-mta/transport/alice@relay.example.com", `"smtp:[relay.example.com]:2525"`},
		{"/stack/legacy-mta/transport/alice@mxrelay.example.com", `"smtp:mxrelay.example.com"`},
		{"/stack/legacy-mta/transport/alice@lmtprelay.example.com", `"lmtp:imap.example.com:2525"`},
	}
	for _, c := range cases {
		if code, body := doGet(t, app, c.path); code != 200 || body != c.want {
			t.Errorf("%s: got %d %q, want %q", c.path, code, body, c.want)
	...
	for _, path := range []string{
		"/stack/legacy-mta/transport/alice@unknown.example.com",
		"/stack/legacy-mta/transport/*",
	} {
		if code, _ := doGet(t, app, path); code != 404 {
			t.Errorf("%s: got %d, want 404", path, code)
		}
...
func TestPostfixSenderLoginContract(t *testing.T) {
	...
	code, body := doGet(t, app, "/stack/legacy-mta/sender/login/alice@example.com")
	if code != 200 {
	...
	code, body = doGet(t, app, "/stack/legacy-mta/sender/login/anyone@spoof.example.com")
	if code != 200 {
	...
	if code, _ := doGet(t, app, "/stack/legacy-mta/sender/login/nobody@unknown.example.com"); code != 404 {
...
func TestPostfixSRSAndRateContract(t *testing.T) {
	...
	if code, _ := doGet(t, app, "/stack/legacy-mta/recipient/map/alice@example.com"); code != 404 {
		t.Errorf("recipient/map plain: got %d, want 404", code)
	}
	if code, _ := doGet(t, app, "/stack/legacy-mta/sender/map/alice@example.com"); code != 404 {
		t.Errorf("sender/map served domain: got %d, want 404", code)
	}

	// External sender is SRS-rewritten, then the recipient map restores it.
	code, body := doGet(t, app, "/stack/legacy-mta/sender/map/john@example.org")
	...
	code, body = doGet(t, app, "/stack/legacy-mta/recipient/map/"+srs)
	if code != 200 || body != `"john@example.org"` {
		t.Fatalf("recipient/map SRS: got %d %q", code, body)
	}

	// Rate endpoint answers 404 while under the limit (legacy MTA treats a hit
	// as a temporary failure only when the JSON smtp code is returned).
	if code, _ := doGet(t, app, "/stack/legacy-mta/sender/rate/alice@example.com"); code != 404 {
		t.Errorf("sender/rate under limit: got %d, want 404", code)
	}
	if code, _ := doGet(t, app, "/stack/legacy-mta/sender/rate/nobody@example.com"); code != 404 {
...
func TestDovecotContract(t *testing.T) {
	...
	if code, body := doGet(t, app, "/stack/legacy-imap/passdb/alice@example.com"); code != 200 ||
		body != `{"allow_real_nets":"192.168.206.0/24","nopassword":"Y","password":null}` {
		t.Fatalf("passdb: got %d %q", code, body)
	}
	if code, body := doGet(t, app, "/stack/legacy-imap/userdb/alice@example.com"); code != 200 ||
		body != `{"quota_rule":"*:bytes=1000000000"}` {
		t.Fatalf("userdb: got %d %q", code, body)
	}
	if code, _ := doGet(t, app, "/stack/legacy-imap/passdb/nobody@example.com"); code != 404 {
		t.Fatalf("passdb missing: got %d", code)
	}

	// userdb iteration: set equality (SQL order is not guaranteed).
	code, body := doGet(t, app, "/stack/legacy-imap/userdb/")
	if code != 200 {
	...
	if code, body := doPost(t, app, "/stack/legacy-imap/quota/bytes/alice@example.com", "2048"); code != 200 {
	...
	if code, body := doGet(t, app, "/stack/legacy-imap/sieve/name/default/alice@example.com"); code != 200 || body != `"default"` {
		t.Fatalf("sieve name: got %d %q", code, body)
	}
	code, body = doGet(t, app, "/stack/legacy-imap/sieve/data/default/alice@example.com")
	if code != 200 {
...
func TestDovecotSieveWhitelistBlacklist(t *testing.T) {
	...
	_, body := doGet(t, app, "/stack/legacy-imap/sieve/data/default/alice@example.com")
	var script string
	if err := json.Unmarshal([]byte(body), &script); err != nil {
		t.Fatalf("sieve data unmarshal: %v", err)
	}
...
func TestIDNAContract(t *testing.T) {
	...
	// queried.
	if code, body := doGet(t, app, "/stack/legacy-mta/domain/"+punycode); code != 200 || body != `"`+punycode+`"` {
		t.Fatalf("domain punycode: got %d %q", code, body)
	}
	if code, body := doGet(t, app, "/stack/legacy-mta/domain/"+unicode); code != 200 || body != `"`+punycode+`"` {
	...
	if code, body := doGet(t, app, "/stack/legacy-mta/alias/team@example.com"); code != 200 ||
		body != `"x@`+punycode+`"` {
...
func TestDistributionGroupExpansion(t *testing.T) {
	...
	// The legacy MTA alias map exposes the same flat list (engine contract).
	code, body = doGet(t, app, "/stack/legacy-mta/alias/all@example.com")
	if code != 200 {
		t.Fatalf("legacy MTA alias: got %d %q", code, body)
	}
	var joined string
	if err := json.Unmarshal([]byte(body), &joined); err != nil {
		t.Fatalf("unmarshal legacy MTA: %v (%q)", err, body)
	}
	if parts := strings.Split(joined, ","); len(parts) != 4 {
		t.Fatalf("legacy MTA targets = %v, want 4", parts)
	}
}
