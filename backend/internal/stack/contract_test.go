...
// Contract tests for the /internal API consumed by the mail stack (legacy MTA /
// legacy IMAP / nginx) through the mail stack's internal bridge. The response formats below
// must stay byte-compatible with the the mail stack's internal API, so these tests pin the
...
func TestPostfixDomainContract(t *testing.T) {
	...
	if code, body := doGet(t, app, "/internal/legacy MTA/domain/example.com"); code != 200 || body != `"example.com"` {
		t.Fatalf("domain: got %d %q", code, body)
	}
	if code, body := doGet(t, app, "/internal/legacy MTA/domain/alt.example.com"); code != 200 || body != `"example.com"` {
		t.Fatalf("alternative: got %d %q", code, body)
	}
	if code, _ := doGet(t, app, "/internal/legacy MTA/domain/%5B1.2.3.4%5D"); code != 404 {
		t.Fatalf("bracketed domain: got %d", code)
	}
	if code, _ := doGet(t, app, "/internal/legacy MTA/domain/unknown.example.com"); code != 404 {
...
func TestPostfixMailboxContract(t *testing.T) {
	...
	if code, body := doGet(t, app, "/internal/legacy MTA/mailbox/alice@example.com"); code != 200 || body != `"alice@example.com"` {
		t.Fatalf("mailbox: got %d %q", code, body)
	}
	if code, _ := doGet(t, app, "/internal/legacy MTA/mailbox/nobody@example.com"); code != 404 {
...
func TestPostfixAliasContract(t *testing.T) {
	...
	cases := []struct {
		...
		{"/internal/legacy MTA/alias/team@example.com", `"alice@example.com,bob@example.com"`},
		{"/internal/legacy MTA/alias/team+detail@example.com", `"alice+detail@example.com,bob+detail@example.com"`},
		{"/internal/legacy MTA/alias/team%2Bdetail@example.com", `"alice+detail@example.com,bob+detail@example.com"`},
		{"/internal/legacy MTA/alias/alice@example.com", `"alice@example.com"`},
		{"/internal/legacy MTA/alias/example.com", `"example.com"`},
		{"/internal/legacy MTA/alias/wildcard@example.com", `"alice@example.com"`},
	}
	for _, c := range cases {
	...
	for _, path := range []string{
		"/internal/legacy MTA/alias/nobody@unknown.example.com",
		"/internal/legacy MTA/alias/%22quoted%22@example.com",
		"/internal/legacy MTA/alias/a%40b@example.com",
	} {
...
func TestPostfixTransportContract(t *testing.T) {
	...
	cases := []struct {
		...
		{"/internal/legacy MTA/transport/alice@relay.example.com", `"smtp:[relay.example.com]:2525"`},
		{"/internal/legacy MTA/transport/alice@mxrelay.example.com", `"smtp:mxrelay.example.com"`},
		{"/internal/legacy MTA/transport/alice@lmtprelay.example.com", `"lmtp:imap.example.com:2525"`},
	}
	for _, c := range cases {
		if code, body := doGet(t, app, c.path); code != 200 || body != c.want {
			t.Errorf("%s: got %d %q, want %q", c.path, code, body, c.want)
	...
	for _, path := range []string{
		"/internal/legacy MTA/transport/alice@unknown.example.com",
		"/internal/legacy MTA/transport/*",
	} {
		if code, _ := doGet(t, app, path); code != 404 {
			t.Errorf("%s: got %d, want 404", path, code)
		}
...
func TestPostfixSenderLoginContract(t *testing.T) {
	...
	code, body := doGet(t, app, "/internal/legacy MTA/sender/login/alice@example.com")
	if code != 200 {
	...
	code, body = doGet(t, app, "/internal/legacy MTA/sender/login/anyone@spoof.example.com")
	if code != 200 {
	...
	if code, _ := doGet(t, app, "/internal/legacy MTA/sender/login/nobody@unknown.example.com"); code != 404 {
...
func TestPostfixSRSAndRateContract(t *testing.T) {
	...
	if code, _ := doGet(t, app, "/internal/legacy MTA/recipient/map/alice@example.com"); code != 404 {
		t.Errorf("recipient/map plain: got %d, want 404", code)
	}
	if code, _ := doGet(t, app, "/internal/legacy MTA/sender/map/alice@example.com"); code != 404 {
		t.Errorf("sender/map served domain: got %d, want 404", code)
	}

	// External sender is SRS-rewritten, then the recipient map restores it.
	code, body := doGet(t, app, "/internal/legacy MTA/sender/map/john@example.org")
	...
	code, body = doGet(t, app, "/internal/legacy MTA/recipient/map/"+srs)
	...
	if code, _ := doGet(t, app, "/internal/legacy MTA/sender/rate/alice@example.com"); code != 404 {
		t.Errorf("sender/rate under limit: got %d, want 404", code)
	}
	if code, _ := doGet(t, app, "/internal/legacy MTA/sender/rate/nobody@example.com"); code != 404 {
...
func TestDovecotContract(t *testing.T) {
	...
	if code, body := doGet(t, app, "/internal/legacy IMAP/passdb/alice@example.com"); code != 200 ||
		body != `{"allow_real_nets":"192.168.206.0/24","nopassword":"Y","password":null}` {
		t.Fatalf("passdb: got %d %q", code, body)
	}
	if code, body := doGet(t, app, "/internal/legacy IMAP/userdb/alice@example.com"); code != 200 ||
		body != `{"quota_rule":"*:bytes=1000000000"}` {
		t.Fatalf("userdb: got %d %q", code, body)
	}
	if code, _ := doGet(t, app, "/internal/legacy IMAP/passdb/nobody@example.com"); code != 404 {
		t.Fatalf("passdb missing: got %d", code)
	}

	// userdb iteration: set equality (SQL order is not guaranteed).
	code, body := doGet(t, app, "/internal/legacy IMAP/userdb/")
	if code != 200 {
	...
	if code, body := doPost(t, app, "/internal/legacy IMAP/quota/bytes/alice@example.com", "2048"); code != 200 {
	...
	if code, body := doGet(t, app, "/internal/legacy IMAP/sieve/name/default/alice@example.com"); code != 200 || body != `"default"` {
		t.Fatalf("sieve name: got %d %q", code, body)
	}
	code, body = doGet(t, app, "/internal/legacy IMAP/sieve/data/default/alice@example.com")
	if code != 200 {
...
func TestIDNAContract(t *testing.T) {
	...
	// queried.
	if code, body := doGet(t, app, "/internal/legacy MTA/domain/"+punycode); code != 200 || body != `"`+punycode+`"` {
		t.Fatalf("domain punycode: got %d %q", code, body)
	}
	if code, body := doGet(t, app, "/internal/legacy MTA/domain/"+unicode); code != 200 || body != `"`+punycode+`"` {
		t.Fatalf("domain unicode: got %d %q", code, body)
	}

	// rspamd vault advertises the punycode selector domain (the the mail stack parity).
	...
	if code, body := doGet(t, app, "/internal/legacy MTA/alias/team@example.com"); code != 200 ||
		body != `"x@`+punycode+`"` {
		t.Fatalf("alias destination: got %d %q", code, body)
	}

	// local_domains passes through the stored names (the the mail stack behaviour).
	code, body := doGet(t, app, "/internal/rspamd/local_domains")
