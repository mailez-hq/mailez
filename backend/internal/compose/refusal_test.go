...
// sendJSON posts one /mail/send body and returns its status and response text.
// Named apart from the package's other helper so both can be compiled into the
// same test binary.
func sendJSON(t *testing.T, app interface {
...
func TestSendRejectsOversizedAttachment(t *testing.T) {
	...
	status, body := sendJSON(t, app, `{"to":["b@example.com"],"body":"hi","attachments":[{"filename":"big.bin","content_type":"application/octet-stream","data":"`+big+`"}]}`)
...
func TestSendMapsRecipientRefusal(t *testing.T) {
	...
	status, body := sendJSON(t, app, `{"to":["not-an-email"],"body":"hi"}`)
	if status != http.StatusUnprocessableEntity {
		t.Fatalf("status = %d (%s), want 422", status, body)
	}
...
func TestSendMapsEngineDown(t *testing.T) {
	...
	status, body := sendJSON(t, app, `{"to":["b@example.com"],"body":"hi"}`)
