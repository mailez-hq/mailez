...
func (h *Handler) registerDovecot(r fiber.Router) {
	r.Get("/legacy IMAP/passdb/:email", h.dovecotPassdb)
	r.Get("/legacy IMAP/userdb/", h.dovecotUserdbList)
	r.Get("/legacy IMAP/userdb/:email", h.dovecotUserdb)
	r.Post("/legacy IMAP/quota/:ns/:email", h.dovecotQuota)
	r.Get("/legacy IMAP/sieve/name/:script/:email", h.dovecotSieveName)
	r.Get("/legacy IMAP/sieve/data/default/:email", h.dovecotSieveData)
}

// dovecotPassdb tells legacy IMAP to accept the auth done by the nginx proxy.
...
// dovecotUserdbList returns all enabled users (legacy IMAP userdb iteration).
func (h *Handler) dovecotUserdbList(c *fiber.Ctx) error {
	var emails []string
	if err := h.DB.Model(&models.User{}).Where("enabled = ?", true).Pluck("email", &emails).Error; err != nil {
...
func (h *Handler) dovecotQuota(c *fiber.Ctx) error {
	...
	if err := h.DB.Model(&u).Update("quota_bytes_used", used).Error; err != nil {
		return c.Status(fiber.StatusInternalServerError).JSON(fiber.Map{"error": err.Error()})
	}
	return c.JSON(nil)
}
