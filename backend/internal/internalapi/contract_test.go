...
// legacy IMAP / nginx) through mainstream suite's podop bridge. The response formats below
// must stay byte-compatible with mainstream suite's internal API, so these tests pin the
...
func TestIDNAContract(t *testing.T) {
	...
	// rspamd vault advertises the punycode selector domain (mainstream suite parity).
	...
	// local_domains passes through the stored names (mainstream suite behaviour).
	code, body := doGet(t, app, "/internal/rspamd/local_domains")
