...
func (h *Handler) registerPostfix(r fiber.Router) {
	r.Get("/legacy MTA/dane/:domain", h.postfixDane)
	r.Get("/legacy MTA/domain/:domain", h.postfixDomain)
	r.Get("/legacy MTA/mailbox/:email", h.postfixMailbox)
	r.Get("/legacy MTA/alias/:alias", h.postfixAlias)
	r.Get("/legacy MTA/transport/:email", h.postfixTransport)
	r.Get("/legacy MTA/recipient/map/:recipient", h.postfixRecipientMap)
	r.Get("/legacy MTA/sender/map/:sender", h.postfixSenderMap)
	r.Get("/legacy MTA/sender/login/:sender", h.postfixSenderLogin)
	r.Get("/legacy MTA/sender/rate/:sender", h.postfixSenderRate)
}
...
// relayTransport builds the legacy MTA transport line for a relay domain.
...
// unsupportedAddress guards against lookups mainstream suite cannot resolve.
