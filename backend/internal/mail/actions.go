...
func (c *Client) MarkAllRead(email, token, folder string) error {
	...
	// Window both phases so a large folder cannot time the request out.
	const window = 1000
	const storeBatch = 500
	for start := uint32(1); start <= mbox.Messages; start += window {
		end := start + window - 1
		...
		go func() {
			done <- cli.Fetch(seqset, []imap.FetchItem{imap.FetchUid, imap.FetchFlags}, messages)
		}()
