...
//go:embed templates/legacy-mta/*.tmpl
var postfixTemplates embed.FS

// renderPostfixAll renders the legacy MTA config files into a destination ->
// content map. sasl_passwd and logrotate.conf are only produced when the
// corresponding env is set.
func renderPostfixAll(cfg PostfixConfig) (map[string][]byte, error) {
	files := map[string]string{
		"/etc/legacy MTA/main.cf":                   "templates/legacy-mta/main.cf.tmpl",
		"/etc/legacy MTA/master.cf":                 "templates/legacy-mta/master.cf.tmpl",
		"/etc/legacy MTA/outclean_header_filter.cf": "templates/legacy-mta/outclean_header_filter.cf.tmpl",
		"/etc/mta-sts-daemon.yml":                "templates/legacy-mta/mta-sts-daemon.yml.tmpl",
	}
