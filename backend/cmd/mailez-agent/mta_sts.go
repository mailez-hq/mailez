...
// MTA-STS policy daemon for legacy MTA, a Go port of mta-sts-resolver
...
// serveMTASTS loads the rendered /etc/mta-sts-daemon.yml and serves legacy MTA
...
func serveMTASTS(ctx context.Context, configPath string) error {
	...
	// The agent runs as root but legacy MTA connects as its own user.
...
func (d *stsDaemon) fetchPolicy(domain string) (stsPolicy, bool) {
	...
	req.Header.Set("User-Agent", "mta-sts-resolver")
