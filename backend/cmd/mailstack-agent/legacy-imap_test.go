...
func TestDovecotRender(t *testing.T) {
	...
	data, err := renderDovecotTemplate("templates/legacy-imap/legacy-imap.conf.tmpl", cfg)
	if err != nil {
...
func TestDictProtocol(t *testing.T) {
	...
	handler := agent.NewDictHandler(map[string]string{
		"auth": srv.URL + "/internal/legacy IMAP/{}",
	})
	...
	if gotPath != "/internal/legacy IMAP/quota/user@example.com/user@example.com" {
	...
	if gotPath != "/internal/legacy IMAP/passdb/e2e@example.com" {
	...
	if gotPath != "/internal/legacy IMAP/userdb/e2e@example.com" {
	...
	if gotPath != "/internal/legacy IMAP/sieve/name/default/e2e@example.com" {
	...
	if gotPath != "/internal/legacy IMAP/sieve/data/default/e2e@example.com" {
