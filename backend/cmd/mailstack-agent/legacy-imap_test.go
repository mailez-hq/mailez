...
func TestDictProtocol(t *testing.T) {
	...
	handler := agent.NewDictHandler(map[string]string{
		"auth": srv.URL + "/stack/legacy-imap/{}",
	})
	...
	if gotPath != "/stack/legacy-imap/quota/user@example.com/user@example.com" {
	...
	if gotPath != "/stack/legacy-imap/passdb/e2e@example.com" {
	...
	if gotPath != "/stack/legacy-imap/userdb/e2e@example.com" {
	...
	if gotPath != "/stack/legacy-imap/sieve/name/default/e2e@example.com" {
	...
	if gotPath != "/stack/legacy-imap/sieve/data/default/e2e@example.com" {
