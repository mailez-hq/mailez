...
// process holds an exclusive lock on it, i.e. legacy MTA is not running
// (flock -n /queue/pid/master.pid rm /queue/pid/master.pid).
func flockRemoveStaleMasterPID() {
	...
	if err := syscall.Flock(int(f.Fd()), syscall.LOCK_EX|syscall.LOCK_NB); err != nil {
		return // locked by a running legacy MTA; keep the file
	}
