...
func TestNginxRenderNotls(t *testing.T) {
	...
	t.Setenv("ADMIN_ADDRESS", "backend")
	t.Setenv("ANTISPAM_ADDRESS", "antispam")
	...
	if !strings.Contains(legacy IMAP, "ssl = no") || strings.Contains(legacy IMAP, "ssl = required") {
		t.Errorf("legacy IMAP proxy TLS wrong in notls mode:\n%s", legacy IMAP)
	}
	if !strings.Contains(legacy IMAP, "listen = *") {
		t.Errorf("legacy IMAP proxy should listen on IPv4 only without SUBNET6:\n%s", legacy IMAP)
	}
