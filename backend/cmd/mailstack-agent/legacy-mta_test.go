...
func TestPostfixRender(t *testing.T) {
	...
	main := string(files["/etc/legacy MTA/main.cf"])
	for _, want := range []string{
		...
		"socketmap:unix:/tmp/mta-sts.socket:legacy MTA",
	} {
		if !strings.Contains(main, want) {
			t.Errorf("main.cf missing %q", want)
		}
	}
	master := string(files["/etc/legacy MTA/master.cf"])
...
func TestPostfixSocketmapProtocol(t *testing.T) {
	...
	urlFor := func(table, key string) string {
		if suffix, ok := postfixTables[table]; ok {
			return srv.URL + "/internal/legacy MTA/" + suffix + key
		}
		return ""
	}
	...
	if gotPath != "/internal/legacy MTA/alias/team@example.com" {
