...
- ActiveSync (EAS) server at `/Microsoft-Server-ActiveSync` plus
  ...
  invitations and ResolveRecipients; configure iOS/mainstream clients with the mailbox
...
- Outbound DLP and approval workflow: keyword/regex rules, hold-for-approval,
  approver console and expiry auto-reject (enterprise-style 审批)
...
- ActiveSync calendar & contacts sync: iOS/mainstream clients can now sync the
